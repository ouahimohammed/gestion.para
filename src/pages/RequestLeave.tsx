...
import { Plus, Calendar, FileText, Clock, CheckCircle, XCircle, User, Building2, Trash2, Award } from 'lucide-react';
...
export function RequestLeave() {
  ...
  const [soldeConge, setSoldeConge] = useState(0); // État pour le solde de congé

  useEffect(() => {
    if (userProfile) {
      fetchMyLeaves();
      fetchSoldeConge(); // Charger le solde de congé
    }
  }, [userProfile]);
  ...
  try {
    ...
    // Vérifier si le solde est suffisant - recherche dans employes
    const q = query(
      collection(db, 'employes'), 
      where('userId', '==', userProfile.uid)
    );
...
const handleCancelRequest = async (leaveId, leaveStatus, leaveDays) => {
  ...
  try {
    // Si la demande était acceptée, restaurer le solde
    if (leaveStatus === 'accepte') {
      await runTransaction(db, async (transaction) => {
        // Trouver le document employe par userId
        const q = query(
          collection(db, 'employes'), 
          where('userId', '==', userProfile.uid)
        );
  ...
  return (
    <div className="space-y-6">
      {/* Header avec solde de congé */}
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Demande de Congé</h1>
          <div className="flex items-center mt-2 text-lg text-green-600">
            <Award className="h-5 w-5 mr-2" />
            <span className="font-semibold">Solde disponible: {soldeConge} jours</span>
          </div>
        </div>
      ...
      {showForm && (
        <Card className="border-blue-200 bg-blue-50">
          <CardHeader>
            <CardTitle className="text-blue-900">Nouvelle demande de congé</CardTitle>
            <div className="flex items-center text-sm text-blue-700">
              <Award className="h-4 w-4 mr-1" />
              Solde disponible: {soldeConge} jours
            </div>
          </CardHeader>
          <CardContent>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                      <div className="flex items-center text-gray-600 mb-1">
                        ...
                        <span className="text-sm font-medium">Période demandée</span>
                      </div>
