...
import { Calendar, User, Building2, Clock, CheckCircle, XCircle, Plus, Search, Filter, FileText, CalendarDays, UserCheck, ArrowLeft, Trash2 } from 'lucide-react';
...
export function MarkAbsence() {
  ...
  const getSortIcon = (key) => {
    if (sortConfig.key !== key) return <span className="h-4 w-4 opacity-30">↕</span>;
    return sortConfig.direction === 'asc' ? <span className="h-4 w-4">↑</span> : <span className="h-4 w-4">↓</span>;
  };
        ...
        <CardContent className="p-0">
          {sortedAbsences.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              {(userProfile?.role === 'super_admin' || userProfile?.role === 'responsable') && (
                <Button onClick={() => setShowEmployeeDialog(true)} className="rounded-lg bg-blue-600 hover:bg-blue-700">
